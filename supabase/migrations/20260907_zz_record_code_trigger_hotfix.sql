...
-- The shared trigger also runs for customers and suppliers, which do not have a
-- status column. Read/write the optional field through JSON so PostgreSQL never
-- resolves NEW.status for those table row types.
...
begin
  ...
  if tg_table_name = 'purchase_orders' then
    if new.supplier_id is null then
      ...
      select s.name into v_partner_name
    ...
    if new.customer_id is null then
      ...
      select c.name into v_partner_name
