...
export default function ReturnForm({ open, businessId, initialType = 'sales', onClose, onSave }) {
  ...
  return (
    ...
    <form id="return-form" className="space-y-6" onSubmit={submit}>
      ...
      <fieldset>
        <legend className="form-section-title"><PackagePlus size={18} /> {itemLabel} <span className="text-rose-500">*</span></legend>
      ...
      <fieldset>
        ...
        {form.refundNow && <div className="mt-4 form-grid"><Field label={type === 'sales' ? 'Tài khoản chi hoàn tiền' : 'Tài khoản nhận tiền'} required><select className="field" value={form.accountId} onChange={(event) => updateForm('accountId', event.target.value)} disabled={!accounts.length || saving} required><option value="">{accounts.length ? 'Chọn tài khoản' : 'Chưa có tài khoản tiền'}</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></Field><Field label="Phương thức"><select className="field" value={form.paymentMethod} onChange={(event) => updateForm('paymentMethod', event.target.value)}><option value="cash">Tiền mặt</option><option value="bank">Chuyển khoản</option><option value="card">Thẻ</option><option value="other">Khác</option></select></Field></div>}
        <div className="mt-4 form-grid"><Field label="Lý do trả hàng" required><input className="field" value={form.reason} onChange={(event) => updateForm('reason', event.target.value)} placeholder="Ví dụ: Hàng lỗi, giao nhầm..." required /></Field><Field label="Ghi chú"><textarea className="field min-h-20 resize-y" value={form.note} onChange={(event) => updateForm('note', event.target.value)} placeholder="Thông tin bổ sung cho phiếu trả" /></Field></div>
      </fieldset>
