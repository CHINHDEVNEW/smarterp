      ...
      <form id="production-order-form" className="space-y-6" onSubmit={submit}>
        <fieldset>
          ...
          <div className="form-grid">
            ...
            <Field label="Ngày lập" required><input className="field" type="date" value={values.order_date} onChange={(event) => update('order_date', event.target.value)} disabled={saving} required /></Field>
            ...
            {selectedBom ? <Field label="Sản phẩm đầu ra"><div className="field bg-slate-50 font-bold text-slate-700">{selectedBom.output_product_name} · {selectedBom.output_unit}</div></Field> : <Field label="Sản phẩm đầu ra" required><select className="field" value={values.output_product_id} onChange={(event) => update('output_product_id', event.target.value)} disabled={saving} required><option value="">Chọn thành phẩm</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} · {product.code || product.unit}</option>)}</select></Field>}
            <Field label="Sản lượng kế hoạch" required><input className="field text-right" type="number" min="0.001" step="0.001" value={values.planned_quantity} onChange={(event) => update('planned_quantity', event.target.value)} disabled={saving} required /></Field>
