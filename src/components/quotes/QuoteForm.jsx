...
import { CalendarDays, FileText, Minus, PackagePlus, Plus, Save, ShoppingCart, Trash2, UserRound } from 'lucide-react'
...
const initialOrder = { customer_id: '', quote_date: localDateKey(), valid_until: '', discount: '0', shipping_fee: '0', vat_rate: '0', note: '' }

export default function QuoteForm({ open, businessId, onClose, onSave }) {
  ...
  useEffect(() => {
    ...
    setOrder({ ...initialOrder, quote_date: localDateKey(), vat_rate: String(defaultVat) })
    setItems([])
    ...
    Promise.all([listProducts(businessId), listCustomersWithSummary(businessId)])
      ...
      .catch((loadError) => setError(loadError.message || 'Không tải được dữ liệu lập báo giá.'))
  ...
  async function submit(event) {
    ...
    setError('')
    if (!items.length) return setError('Vui lòng thêm ít nhất một sản phẩm vào báo giá.')
    if (items.some((item) => Number(item.quantity) <= 0 || Number(item.unit_price) < 0)) return setError('Số lượng và đơn giá phải hợp lệ.')
    ...
    try {
      ...
      setError(saveError.message || 'Không thể tạo báo giá.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Modal
      ...
      title="Tạo báo giá"
      description="Báo giá không ảnh hưởng tồn kho cho đến khi được chuyển thành đơn bán."
      size="lg"
      icon={FileText}
      tone="sky"
      ...
      footer={
        <div className="flex w-full flex-col-reverse gap-2 sm:w-auto sm:flex-row sm:items-center sm:justify-end">
          ...
          <button className="btn-primary w-full sm:w-auto" type="submit" form="quote-form" disabled={saving || loadingData}>
            <Save size={17} />
            <span>{saving ? 'Đang tạo...' : 'Lưu báo giá'}</span>
          </button>
        </div>
      }
    >
      <form id="quote-form" className="space-y-6" onSubmit={submit}>
        <fieldset>
          <legend className="form-section-title"><UserRound size={18} /> Khách hàng và thời hạn</legend>
          <div className="form-grid">
            <Field label="Khách hàng" className="sm:col-span-2">
              <select className="field" value={order.customer_id} onChange={(event) => updateOrder('customer_id', event.target.value)} disabled={loadingData}>
            ...
            <Field label="Có hiệu lực đến">
              <input className="field" type="date" min={order.quote_date} value={order.valid_until} onChange={(event) => updateOrder('valid_until', event.target.value)} />
            </Field>
          </div>
        </fieldset>
          ...
          <div className="flex gap-2">
            <select className="field min-w-0 flex-1" value={selectedProductId} onChange={(event) => setSelectedProductId(event.target.value)} disabled={loadingData}>
              ...
              {products.map((product) => (
                <option key={product.id} value={product.id}>{product.name} · {formatCurrency(product.sale_price)}</option>
              ))}
            </select>
                ...
                <div className="p-4" key={item.product_id}>
                  ...
                  <div className="mt-3 grid grid-cols-[1fr_1.5fr] gap-3 sm:grid-cols-[140px_1fr_auto] sm:items-end">
                    ...
                    <div className="col-span-2 text-right sm:col-span-1 sm:min-w-32">
                      ...
                      <p className="tabular-nums mt-2 text-sm font-extrabold text-slate-900">{formatCurrency((Number(item.quantity) || 0) * (Number(item.unit_price) || 0))}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <fieldset>
          <legend className="form-section-title"><CalendarDays size={18} /> Tổng báo giá</legend>
          <div className="form-grid">
            ...
            <Field label="Ghi chú">
              <input className="field" value={order.note} onChange={(event) => updateOrder('note', event.target.value)} placeholder="Ghi chú trên báo giá" />
            </Field>
          </div>
          <div className="mt-4 space-y-2 rounded-2xl bg-slate-950 p-5 text-sm text-white">
            ...
            <SummaryRow label="Phí giao hàng" value={formatCurrency(shippingFee)} muted />
            <SummaryRow label={`VAT (${Number(order.vat_rate) || 0}%)`} value={formatCurrency(vatAmount)} muted />
            <div className="my-3 border-t border-white/10" />
            <SummaryRow label="Tổng báo giá" value={formatCurrency(total)} strong />
          </div>
        </fieldset>
